...
void AddItem(_tagItem* pWeaponStore, _tagItem* pArmorStore)
{
	...
	//사용할 아이템 배열 따로 넣어주었다. 아이템을 추가할때에도
	//추가할 상점을 선택하고 아이템의 난이도를 선택했다.
	//그러므로 복잡하기 때문에 여기에서 추가할 아이템의 메모리
	// 주소를 얻어와서 거기에 추가해주면 쉽게 처리 가능하다.
	if (iStore == SM_WEAPON)
	{
...
void Fix(_tagItem* pWeapon, _tagItem* pArmor)
{
	...
	for (int i = 0; i < STORE_ARMOR_MAX; ++i)
	{
		...
		pWeapon[i] = pArmor[i];
		pArmor[i] = tTemp[i];
		strcpy_s(pWeapon[i].strTypeName, NAME_SIZE, "무기");
		strcpy_s(pArmor[i].strTypeName, NAME_SIZE, "방어구");
	}
	cout << "Fix Done!" << endl;
	system("pause");
}
