...
//메인메뉴를 화면에 보여주고 입력받은 메뉴를 반환한다.
//입력 오류일 경우 혹은 잘못된 번호 입력시 MM_NONE 반환.
int OutputMainMenu()
{
	system("cls");
	cout << "*************************** 로비 ***************************" << endl;
...
bool LoadPlayer(_tagPlayer* pPlayer)
{
	...
	if (pFile)
	{
		...
		//공격력을 읽어온다.
		fread(&pPlayer->iAttackMin, 1, 4, pFile);
		fread(&pPlayer->iAttackMax, 1, 4, pFile);
		//방어력을 읽어온다.
		fread(&pPlayer->iArmorMin, 1, 4, pFile);
		fread(&pPlayer->iArmorMax, 1, 4, pFile);
...
bool SavePlayer(_tagPlayer* pPlayer)
{
	...
	if (pFile)
	{
		...
		//공격력을 저장한다.
		fwrite(&pPlayer->iAttackMin, 1, 4, pFile);
		fwrite(&pPlayer->iAttackMax, 1, 4, pFile);
		//방어력을 저장한다.
		fwrite(&pPlayer->iArmorMin, 1, 4, pFile);
		fwrite(&pPlayer->iArmorMax, 1, 4, pFile);
...
void SetMonster(_tagMonster* pMonsterArr) {
	...
	/*
	pMonsterArr[0] = CreateMonster("고블린", 20, 30, 2, 5, 100, 10,
		1, 1000, 500, 1500);
	pMonsterArr[1] = CreateMonster("트롤", 80, 130, 60, 90, 2000, 100,
		5, 7000, 6000, 8000);
	pMonsterArr[2] = CreateMonster("드래곤", 250, 500, 200, 400, 30000,
		20000, 10, 30000, 20000, 50000);
	*/
}

_tagLevelUpStatus CreateLvUpStatus(int iAttackMin,
	int iAttackMax, int iArmorMin, int iArmorMax, int iHPMin, int iHPMax,
	...
	int iPrice, int iSell, const char* pDesc)
{
	...
	strcat_s(tItem.strName, pName);
	strcat_s(tItem.strDesc, pDesc);
...
void LoadStore(_tagItem* pWeapon, _tagItem* pArmor)
{
	FILE* pFile = NULL;
...
int main()
{
	...
	*/
	...
	while (bLoop)
	{
		...
		{
		case MM_MAP:
			RunMap(&tPlayer, tMonsterArr); // 맵 관련 루프 시작
			break;
		case MM_STORE:
